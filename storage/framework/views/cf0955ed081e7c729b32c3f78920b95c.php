<?php $__env->startSection('title', 'Управление организациями'); ?>
    ...
    <div class="dark-page-wrapper">
            ...
            <div class="table-responsive">
                <table class="glass-table">
                    ...
                    <?php $__currentLoopData = $organizations; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $org): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr>
                            <td>
                                <div class="fw-bold fs-6 mb-1"><?php echo e($org->name); ?></div>
                                <div style="color: var(--text-secondary); font-size: 0.8rem;"><i class="bi bi-geo-alt me-1"></i><?php echo e($org->address); ?></div>
                            </td>
                            <td>
                                <div class="fw-medium mb-1"><?php echo e($org->user->name); ?></div>
                                <div style="color: var(--text-secondary); font-size: 0.8rem;"><?php echo e($org->user->email); ?></div>
                            </td>
                            <td>
                            ...
                            <td class="text-end">
                                <div class="d-flex justify-content-end gap-2">
                                    <?php if(!$org->is_verified): ?>
                                        <form action="<?php echo e(route('admin.organizations.verify', $org)); ?>" method="POST">
                                            <?php echo csrf_field(); ?> <?php echo method_field('PUT'); ?>
                                            <button type="submit" class="btn-verify-glow"><i class="bi bi-check-lg"></i> Подтвердить</button>
                                        </form>
                                    <?php endif; ?>
                                    <form action="<?php echo e(route('admin.organizations.delete', $org)); ?>" method="POST" onsubmit="return confirm('Организация будет удалена, а владелец станет обычным клиентом. Продолжить?')">
        ...
        <?php if($organizations->hasPages()): ?>
            <div class="custom-pagination-container">
                <div class="pagination-info">Показаны записи с <b><?php echo e($organizations->firstItem()); ?></b> по <b><?php echo e($organizations->lastItem()); ?></b> из <b><?php echo e($organizations->total()); ?></b></div>
                <ul class="custom-pagination">
                    <li class="page-item <?php echo e($organizations->onFirstPage() ? 'disabled' : ''); ?>">
                        <a class="page-link" href="<?php echo e($organizations->previousPageUrl() ?? '#'); ?>"><i class="bi bi-chevron-left me-1"></i> Назад</a>
                    </li>
                    <?php $__currentLoopData = $organizations->getUrlRange(max(1, $organizations->currentPage() - 2), min($organizations->lastPage(), $organizations->currentPage() + 2)); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $page => $url): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <li class="page-item <?php echo e($page == $organizations->currentPage() ? 'active' : ''); ?>">
                            <a class="page-link" href="<?php echo e($url); ?>"><?php echo e($page); ?></a>
                        </li>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    <li class="page-item <?php echo e(!$organizations->hasMorePages() ? 'disabled' : ''); ?>">
                        <a class="page-link" href="<?php echo e($organizations->nextPageUrl() ?? '#'); ?>">Дальше <i class="bi bi-chevron-right ms-1"></i></a>
                    </li>
                </ul>
            </div>
        <?php endif; ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\OSPanel\home\booking-system.local\resources\views/admin/organizations.blade.php ENDPATH**/ ?>