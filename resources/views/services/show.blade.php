            ...
            <div class="row g-5">
                <div class="col-lg-8">
                    <div class="glass-panel mb-4">
                        <div class="badge-row">
                            <span class="neon-badge badge-cat">{{ $service->category->name ?? 'Услуга' }}</span>
                            <span class="neon-badge badge-time"><i class="bi bi-clock me-2"></i>{{ $service->duration }} мин</span>
                        </div>
                        ...
                        <div class="org-box">
                            ...
                            <div class="org-details">
                                <h5>{{ $service->organization->name }}</h5>
                                <p><i class="bi bi-geo-alt text-primary"></i> {{ $service->organization->address }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="glass-panel">
                        ...
                        @if($reviews->count() > 0)
                            <div class="lead text-white mb-4" style="font-weight: 700;">Средний рейтинг: <span class="text-warning">{{ number_format($averageRating, 1) }} ★</span></div>
                            @foreach($reviews as $review)
                                <div class="review-card">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <div class="review-user"><i class="bi bi-person-circle me-2 opacity-50"></i>{{ $review->client->name }}</div>
                                        <div style="color: var(--text-muted); font-size: 0.85rem;">{{ $review->created_at->format('d.m.Y') }}</div>
                                    </div>
                                    <div class="mb-2">
                                        @for($i = 1; $i <= 5; $i++)
                                            <i class="bi bi-star{{ $i <= $review->rating ? '-fill star-filled' : '' }}"></i>
                                        @endfor
                                    </div>
                                    ...
                                    @if($review->reply)
                                        <div class="mt-3 p-3 rounded" style="background: rgba(0, 102, 255, 0.08); border-left: 3px solid var(--accent-primary);">
                                            ...
                                            <div style="color: var(--text-description); font-size: 0.95rem;">{{ $review->reply }}</div>
                                        </div>
                                    @endif
                                </div>
                            @endforeach
                ...
                <div class="col-lg-4">
                    <div class="sticky-booking-card">
                        ...
                        <div class="price-large">{{ number_format($service->price, 0, '', ' ') }} ₽</div>

                        @auth
                            @if(auth()->user()->isClient())
                                <form action="{{ route('bookings.store', $service->id) }}" method="POST" id="bookingForm" class="text-start mt-4">
                                    @csrf
                            ...
                            @endif
                        @else
                            <a href="{{ route('login') }}" class="btn-glow text-center d-block mt-4" style="text-decoration: none;">Войти для записи</a>
                        @endauth
