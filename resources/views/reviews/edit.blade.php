    ...
    <div class="dark-page-wrapper">
        ...
        <div class="glass-panel">
            ...
            <div class="service-info-badge">
                <i class="bi bi-tag-fill me-1"></i> {{ $review->booking->service->title ?? 'Услуга' }}
            </div>
            ...
            <form action="{{ route('reviews.update', $review) }}" method="POST">
                ...
                <div class="mb-5">
                    ...
                    <div class="rating-container">
                        @for($i = 1; $i <= 5; $i++)
                            <input type="radio" name="rating" id="rating{{ $i }}" value="{{ $i }}" class="d-none" {{ $review->rating == $i ? 'checked' : '' }} required>
                            <label class="star-label {{ $review->rating >= $i ? 'active' : '' }}" for="rating{{ $i }}" data-value="{{ $i }}">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="comment" id="comment" class="glass-textarea" placeholder="Расскажите подробнее о качестве услуги и работе мастера..." required minlength="10">{{ old('comment', $review->comment) }}</textarea>
                    @error('comment') <div class="text-danger small mt-2 fw-bold"><i class="bi bi-exclamation-triangle"></i> {{ $message }}</div> @enderror
                </div>
                ...
                <div class="pt-3">
                    ...
                    <a href="{{ route('my-reviews') }}" class="btn-cancel-glass">Отменить изменения</a>
                </div>
            </form>
        </div>
    </div>
