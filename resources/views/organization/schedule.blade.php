...
@section('title', 'Настройка расписания')
    ...
    <div class="dark-page-wrapper">
        ...
        <div class="page-header">
            ...
            <div class="d-flex gap-2">
                <a href="{{ route('organization.services.index') }}" class="btn-nav-glass">
                    <i class="bi bi-arrow-left"></i> К услугам
                </a>
                <a href="{{ route('organization.slots.index', $service->id) }}" class="btn-nav-glass btn-nav-primary">
        ...
        <div class="row g-4">
            <div class="col-xl-5">
                <div class="glass-panel">
                    ...
                    <form action="{{ route('organization.schedule.store', $service->id) }}" method="POST">
                        @csrf
                        ...
                        <div class="days-grid">
                            ...
                            @foreach($days as $val => $name)
                                <div class="day-item">
                                    <input type="checkbox" name="days[]" value="{{ $val }}" id="day{{ $val }}" class="day-checkbox" style="display:none">
                                    <label for="day{{ $val }}" class="day-label">{{ $name }}</label>
                                </div>
                            @endforeach
                        </div>
            ...
            <div class="col-xl-7">
                <div class="glass-panel">
                    <h5 class="panel-title"><i class="bi bi-alarm text-primary"></i> График услуги: {{ $service->title }}</h5>
                    ...
                    @if($schedules->count() > 0)
                        <div class="table-responsive mb-4">
                            <table class="glass-table">
                                ...
                                <tbody>
                                @php $dayNames = [1 => 'Понедельник', 2 => 'Вторник', 3 => 'Среда', 4 => 'Четверг', 5 => 'Пятница', 6 => 'Суббота', 7 => 'Воскресенье']; @endphp
                                @foreach($schedules as $schedule)
                                    <tr>
                                        <td class="fw-bold text-white">{{ $dayNames[$schedule->day_of_week] }}</td>
                                        <td>
                                        ...
                                        <td class="text-white">
                                            @if($schedule->break_start && $schedule->break_end)
                                                <i class="bi bi-clock-history me-1 opacity-50"></i>{{ \Carbon\Carbon::parse($schedule->break_start)->format('H:i') }}-{{ \Carbon\Carbon::parse($schedule->break_end)->format('H:i') }}
                                            @else
                                                <span class="opacity-25">—</span>
                                            @endif
                                        </td>
                                        <td class="text-end">
                                            <form action="{{ route('organization.schedule.destroy', [$service->id, $schedule->day_of_week]) }}" method="POST">
                                                @csrf @method('DELETE')
                        ...
                        <div class="magic-card">
                            ...
                            <form action="{{ route('organization.slots.generate', $service->id) }}" method="POST">
                                @csrf
