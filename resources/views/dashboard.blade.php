...
@section('content')
    ...
    <div class="dark-page-wrapper container py-4">
        ...
        <div class="page-header">
            <h1 class="page-title"><i class="bi bi-person-circle"></i> Привет, {{ auth()->user()->name }}!</h1>
        </div>
                ...
                <div class="stat-card" style="--stat-color: var(--accent-primary); --stat-rgb: 0, 102, 255; --delay: 1;">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h3 class="stat-value">{{ isset($bookings) ? $bookings->count() : 0 }}</h3>
                        </div>
                ...
                <div class="stat-card" style="--stat-color: var(--neon-info); --stat-rgb: 0, 210, 255; --delay: 2;">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h3 class="stat-value">
                                {{ isset($bookings) ? $bookings->whereIn('status', ['pending', 'confirmed'])->count() : 0 }}
                            </h3>
                        </div>
                ...
                <div class="stat-card" style="--stat-color: var(--neon-warning); --stat-rgb: 255, 209, 102; --delay: 3;">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h3 class="stat-value">{{ $waitingListCount ?? 0 }}</h3>
                        </div>
            ...
            <div class="mt-4">
                @if(isset($bookings) && $bookings->count() > 0)
                    @foreach($bookings->take(5) as $booking)
                        <div class="booking-item">
                            <div>
                                <h6 class="booking-title">{{ $booking->service->title }}</h6>
                                <div class="booking-details">
                                    <span><i class="bi bi-building"></i> <span style="color: var(--text-description);">{{ $booking->service->organization->name }}</span></span>
                                    <span><i class="bi bi-calendar3"></i> <span style="color: var(--text-description);">{{ \Carbon\Carbon::parse($booking->booking_date)->format('d.m.Y') }}</span></span>
                                    <span><i class="bi bi-clock"></i> <span style="color: var(--text-description);">{{ \Carbon\Carbon::parse($booking->start_time)->format('H:i') }}</span></span>
                                </div>
                            </div>
                            <div>
                    ...
                    <div class="mt-4 text-center border-top border-secondary border-opacity-10 pt-4">
                        <a href="{{ route('my-bookings') }}" class="btn-link-glow">
                ...
                @else
                    <div class="text-center py-5">
                        ...
                        <a href="{{ route('services.index') }}" class="btn-primary-glow">
                            Найти услугу
                        </a>
                    </div>
                @endif
