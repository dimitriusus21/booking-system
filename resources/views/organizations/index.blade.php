...
@section('title', 'Наши организации')
                ...
                @foreach($organizations as $index => $organization)
                    <div class="col">
                        <div class="org-card" style="animation-delay: {{ $index * 0.1 }}s;">
                            ...
                            <span class="cat-badge">{{ $organization->category->name ?? 'Сервис' }}</span>

                            <h5 class="org-name">{{ $organization->name }}</h5>
                            ...
                            <div class="rating-box">
                                ...
                                <div class="stars-glow">
                                    @for($i = 1; $i <= 5; $i++)
                                        {!! $i <= $roundedRating ? '★' : '<span style="opacity: 0.2">☆</span>' !!}
                                    @endfor
                                </div>

                                @if($organization->reviews_count > 0)
                                    <span class="rating-value">{{ number_format($rating, 1, '.', '') }}</span>
                                    <span class="reviews-count">({{ $organization->reviews_count }} отзывов)</span>
                                @else
                            ...
                            <p class="org-description">
                                {{ Str::limit($organization->description ?? 'Данная организация еще не добавила описание своих услуг и преимуществ.', 120) }}
                            </p>

                            <div class="org-location mt-auto">
                                <i class="bi bi-geo-alt-fill"></i>
                                <span>{{ $organization->address ?? 'Адрес уточняйте' }}</span>
                            </div>

                            <a href="{{ route('organizations.show', $organization) }}" class="btn-org-link">
            ...
            <div class="glass-panel text-center py-5 rounded-4">
                ...
                @auth
                    @if(auth()->user()->isOrganization())
                        <a href="{{ route('organization.profile.create') }}" class="btn-org-link d-inline-flex px-5">Создать профиль</a>
                    @endif
                @endauth
            </div>
        @endif
